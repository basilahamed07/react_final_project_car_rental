...
const fetchCars = async () => {
  try {
    const response = await fetch('http://localhost:8888/car?type=suplux');
    if (!response.ok) {
...
const handleAddOne = async (carid) => {
  ...
  const featchCarItens = () =>{
      axios.get("http://localhost:8888/car?type=suplux").then((referance)=>{
      ...
      cars.map((val, index) => (
          ...
          <div className="card mb-3" id='card'>
            ...
            <div className="card-body">
              ...
              <button key={users.id} onClick={() => handleAddOne(val.carid)} className="btn btn-primary btn-hover float-left">{val.price} Only/-  </button> 
            </div>  
